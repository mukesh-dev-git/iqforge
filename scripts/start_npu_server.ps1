...
$deviceId = ($deviceLine.ToString() -split "\s+")[0].Trim()
Write-Host "Target device: $deviceId" -ForegroundColor Green

# Kill any previous instance
& $adb -s $deviceId shell "pkill -9 -f llama-server"
...
for ($attempt = 1; $attempt -le 40; $attempt++) {
    $health = & $adb -s $deviceId shell "curl -s -m 2 http://127.0.0.1:8080/health"
...
if ($health -match "ok") {
    ...
    & $adb -s $deviceId shell "tail -n 15 /data/local/tmp/llama-server.log"
}
